(() => {
  ...
  function readingQuizPartsMarkup(parts, mode = 'fill') {
    ...
    return values.map((part) => {
      if (part && typeof part === 'object' && part.gap) {
        ...
        return `<span class="reading-gap-wrap" data-reading-gap-wrap="${escapeHtml(gapId)}"><sup>${escapeHtml(gapId)}</sup><input class="reading-gap-input" data-quiz-gap="${escapeHtml(gapId)}" aria-label="Gap ${escapeHtml(gapId)}" autocomplete="off"><span class="reading-gap-feedback" aria-live="polite"></span></span>`;
      }
  ...
  function checkReadingQuizBlock(block, node) {
    ...
    Object.entries(gapAnswers).forEach(([gapId, expected]) => {
      ...
      if (feedback) feedback.textContent = correct ? '✓' : 'Проверь';
    });
    ...
    Object.entries(matchAnswers).forEach(([personId, expected]) => {
      ...
      if (feedback) {
        feedback.className = `reading-match-feedback feedback show ${correct ? 'good' : 'bad'}`;
        feedback.textContent = correct ? 'Верно!' : 'Проверь ответ и попробуй ещё раз.';
      }
    });
